...
<div align="center">
  <img src="https://github.com/gui2604/healthguard-enviromonitor-python-program/assets/128194162/2995b237-4a35-47e6-8dd1-09b51d9ca222" width="700px">
</div>

<p>O programa da uma mensagem de boas-vindas, e realiza a autenticação do usuário, pedindo usuário (usuario123), senha (senha123) e crm (123) passando essas variáveis para a função “fazer_login”, caso correto o usuário terá acesso ao menu, caso contrário uma mensagem de bloqueio de conta será exibida e encerrará o programa.
O menu dá ao usuário 5 opções, 1- Cadastrar um paciente, 2- Alterar os dados de um paciente, 3- Deletar a ficha de um paciente, 4- Consultar os dados de um paciente, 5- Consultar todos os dados de todos os pacientes ou 0- Sair, forçando o usuário a escolher uma dessas 6 opções. A cada operação feita pelo usuário o programa irá perguntar a ele se quer continuar realizando operações. Caso deseje parar, o programa irá gerar um arquivo de banco de dados com o registro de todos os dados dos pacientes cadastrados e encerrará o programa. Para cada operação que o usuário deseja fazer, chamará uma das funções explicadas anteriormente e realizará a operação respectiva.

<strong font-size="20px">Observações Importantes:</strong> Para fins de teste e funcionamento adequado do programa dois arquivos externos acompanham de suporte para duas funções do programa. A função “cadastrar_paciente” consome o arquivo externo “dadosComodos.json”, que simula os dados coletados pelo Arduino e enviados via MQTT no formato “.json”. A função “alterar_paciente” também consome um arquivo externo nos mesmos formatos, mas para simular dados diferentes e um arquivo externo diferente, ela consome o arquivo externo “novosDadosComodos.json”. Por fim, o programa em si ao ser encerrado gera um arquivo externo de banco de dados “dadosPacientes.json” com todo o registro de pacientes armazenados na lista de dicionários de pacientes gerados no programa.
</p>

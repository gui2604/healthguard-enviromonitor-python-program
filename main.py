...
def print_dic(dic):
    for key in dic.keys():
        if type(dic[key]) is dict:
            print(f"últimos dados medidos do/a(dos/das) {key}:")
            print_dic(dic[key])
        else:
            print(f"{key} : {dic[key]}")
    return
...
def alterar_paciente():
    ...
    while True:
        try:
            dado = forca_opcao(f"Qual dado você deseja alterar? Apenas {pacientes[0].keys()}\nR:", [str(key) for key in pacientes[0].keys()])
            tipo = type(pacientes[opcao][dado])
            if tipo is dict:
                with open("novosDadosComodos.json", "r") as arquivo:
                    dados_comodos = json.load(arquivo)
                    pacientes[opcao][dado] = dados_comodos[dado]
            else:
                pacientes[opcao][dado] = tipo(input(f"Diga o/a novo/a {dado}\nR:"))
        except ValueError:
            print(f"Digite um dado válido!")
        except FileNotFoundError:
            print(f"Arquivo não encontrado! Pode estar inválido, verifique o caminho ou formato.")
        except Exception as e:
            print(e)
        else:
            alterar = forca_opcao("quer alterar mais informações desse paciente(s/n)?", ["s", "n"])
            if alterar == "n":
                break
